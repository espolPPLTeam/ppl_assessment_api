...
module.exports = ({ db }) => {
  const proto = {
    ...
    async Actualizar ({ id, nombres, apellidos, carrera, matricula, correo }) {
      let fueActulizado = await db.Estudiantes.Actualizar({ id, nombres, apellidos, carrera, matricula, correo })
      if (fueActulizado) {
